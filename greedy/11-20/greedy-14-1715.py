...
for _ in range(N):
    card_group.append(int(sys.stdin.readline()))

heapq.heapify(card_group)
how_many_compare = 0
while len(card_group) > 1:
    min_group_of_card1 = heapq.heappop(card_group)
    min_group_of_card2 = heapq.heappop(card_group)
    how_many_compare += min_group_of_card1 + min_group_of_card2
    heapq.heappush(card_group, min_group_of_card1 + min_group_of_card2)
    
print(how_many_compare)
